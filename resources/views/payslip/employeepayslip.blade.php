                                    ...
                                    <table class="table table-striped mb-0" id="dataTable1">
                                        ...
                                        @foreach($payslip as $payslip)
                                            <tr>
                                                ...
                                                <td>{{ \App\PaySlip::employee($payslip->employee_id)->name }}</td>
                                                <td>{{ $payslip->salary_month }}</td>
                                                <td>{{ $payslip->basic_salary }}</td>
                                                <td>{{ $payslip->net_payble }}</td>
                                                <td>@if($payslip->status == 1)
                                                        <div class="badge badge-success"><a class="text-white">{{__('Paid')}}</a></div> @else
                                                        <div class="badge badge-danger"><a class="text-white">{{__('Unpaid')}}</a></div>@endif</td>
                                                <td>
                                                    ...
                                                    <a href="{{ route('payslip.pdf',$payslip->id) }}" class="btn btn-sm btn-info btn-round btn-icon">
                                                        {{__('Payslip')}}
                                                    </a>
                                                </td>
                                            </tr>
